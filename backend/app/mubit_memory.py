    ...
    def reset_user_memory(self, *, user_id: str, hard_reset: bool) -> None:
        ...
        try:
            ...
            if hard_reset and hasattr(self._client, "forget"):
                try:
                    self._client.forget(session_id=user_id, query="*", limit=200)  # type: ignore[call-arg]  # ty: ignore[unknown-argument]
                except Exception:  # noqa: BLE001
                    logger.exception(
    ...
    def reflect_session(self, *, user_id: str) -> None:
        ...
        try:
            self._client.reflect(
                session_id=user_id,
                focus="Extract reusable pose/style lessons for future candidate generation.",  # ty: ignore[unknown-argument]
