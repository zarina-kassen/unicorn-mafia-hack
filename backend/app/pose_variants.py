        ...
        for index, spec in enumerate(POSE_VARIANTS, start=1):
            with reference_path.open("rb") as image_file:
                response = client.images.edit(
                    ...
                    size=IMAGE_SIZE,  # type: ignore[arg-type, ty:invalid-argument-type]
                    quality=IMAGE_QUALITY,  # type: ignore[arg-type, ty:invalid-argument-type]
                    input_fidelity=IMAGE_INPUT_FIDELITY,  # type: ignore[arg-type, ty:invalid-argument-type]
                    output_format="jpeg",
                )
