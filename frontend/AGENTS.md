...
```
src/
├── main.tsx                 # React root: Clerk + Query + TooltipProvider + Toaster + RouterProvider
...
├── pages/
│   ├── OnboardingPage.tsx   # Route wrapper → OnboardingScreen
│   └── CameraPage.tsx       # Route wrapper → CameraScreen
├── screens/
│   ├── OnboardingScreen.tsx # shadcn onboarding UI
│   └── CameraScreen.tsx     # Responsive camera + pose gallery
├── components/
│   ├── ui/                  # shadcn primitives (button, card, sheet, …)
│   ├── CameraFrame.tsx      # Video + vignette + shutter flash
│   ├── PoseGallery.tsx      # Mobile sheet / desktop sidebar
│   └── …
...
- **Tailwind CSS 4** + **shadcn/ui** (radix-nova) for UI primitives; global tokens in `index.css`.
- No CSS-in-JS or CSS modules; complex keyframes live in `index.css`.
